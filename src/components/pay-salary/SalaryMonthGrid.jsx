// components/salary/SalaryMonthGrid.jsx
...
function SalaryMonthGrid({ teacherId }) {
  const { data, isLoading, isError } = useTeacherSalaryByTeacherId(teacherId)

  const records = data?.data ?? []

  // Build a map of month -> record
  const recordMap = records.reduce((acc, r) => {
    acc[r.SalaryMonth] = r
    return acc
  }, {})

  // Generate 12 months — current month and 11 before it
...
export default SalaryMonthGrid
