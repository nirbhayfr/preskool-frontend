// components/staff-salary/StaffSalaryCalculationCard.jsx
...
import { useStaffMonthlySummaryById } from '@/hooks/useStaffAttendance'
import { useCreateStaffSalary, useStaffSalaryByStaffId } from '@/hooks/useStaffSalary'
...
function CalcRow({ label, value, color = '', isTotal = false }) {
  return (
    <div
      className={`flex items-center justify-between py-2 ${isTotal ? 'font-semibold text-base' : 'text-sm'}`}
    >
...
function StaffSalaryCalculationCard({ staffId, baseSalary }) {
  const [month, setMonth] = useState(getCurrentMonth)
  const [allowance, setAllowance] = useState(0)
  const [extraDeduction, setExtraDeduction] = useState(0)
  const [paidLeaves, setPaidLeaves] = useState(0)

  const { data, isLoading, isError } = useStaffMonthlySummaryById(staffId, month)
  const { data: salaryData, isLoading: isSalaryLoading } =
    useStaffSalaryByStaffId(staffId)
  const { mutate: createSalary, isPending: isCreating } = useCreateStaffSalary()
  ...
  if (isLoading || isSalaryLoading) return <StaffSalaryCalculationSkeleton />
  if (isError)
...
export default StaffSalaryCalculationCard

function StaffSalaryCalculationSkeleton() {
  return (
    <Card className="rounded-xl border-muted/60">
      <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
