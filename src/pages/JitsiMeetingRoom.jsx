 ...
 * MeetingPage.jsx
 * ─────────────────────────────────────────────────────────────────────────────
 * Landing page  →  Create OR Join  →  Live Room
 *
 * All controls (mic, camera, screen share, raise hand, chat, participants)
 * are wired directly to the Jitsi IFrame API — they actually work.
 *
 * API routes used:
 *   POST /api/online-meetings/schedule
...
const getUserFromToken = () => {
  try {
    ...
    return {
      id:   p.id ?? p.UserID ?? p.userId ?? p.teacherId ?? null,
      name: p.name ?? p.Name ?? p.username ?? "User",
      role: p.role ?? p.Role ?? "student",
    };
  } catch { return null; }
};
...
const ICONS = {
  ...
  screen:  "M13 3H4a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-3 M8 21h8 M12 17v4 M22 3l-5 5 M17 3h5v5",
...
const Field = ({ label, id, error, icon, ...props }) => (
  <div className="flex flex-col gap-1.5">
    {label && <label htmlFor={id} className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">{label}</label>}
    <div className="relative">
      {icon && <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none"><Ic d={icon} size={15} /></span>}
      <input id={id}
    ...
    {error && <p className="text-[11px] text-destructive flex items-center gap-1"><Ic d={ICONS.warn} size={11} />{error}</p>}
  </div>
);

const Btn = ({ children, loading, variant = "primary", size = "md", className = "", ...props }) => {
  const sz = size === "sm" ? "px-4 py-2 text-xs" : size === "lg" ? "px-6 py-3 text-sm" : "px-5 py-2.5 text-sm";
  const v  = {
  ...
  return (
    <button className={`inline-flex items-center justify-center gap-2 font-semibold rounded-xl transition-all focus:outline-none disabled:opacity-60 disabled:cursor-not-allowed active:scale-[.98] ${sz} ${v[variant]} ${className}`}
      disabled={loading || props.disabled} {...props}>
      {loading ? <><Ic d={ICONS.spin} size={14} cls="animate-spin" />Please wait…</> : children}
    </button>
  );
};

const Badge = ({ children, variant = "primary" }) => {
  const v = { primary: "bg-primary/10 text-primary border-primary/20", success: "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" };
  return <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border ${v[variant]}`}>{children}</span>;
};
...
function LandingPage({ onEnterRoom }) {
  ...
  const handleCreate = async () => {
    ...
    if (!user?.id) { setCErr("Could not read TeacherID from token. Please log in again."); return; }
    setCLoad(true); setCErr(""); setCResult(null);
    try {
      const res  = await authFetch(`${API_BASE}/online-meetings/schedule`, {
        method: "POST",
        body: JSON.stringify({
  ...
  const handleFetch = async () => {
    ...
    setJLoad(true); setJErr(""); setJFetched(false);
    try {
      const res  = await authFetch(`${API_BASE}/online-meetings/list/${jForm.ClassID}/${jForm.SectionID}`);
      const data = await res.json();
      if (!res.ok || !data.success) throw new Error(data.message || "Failed to fetch");
      setMeetings(data.data);
      setJFetched(true);
    } catch (err) { setJErr(err.message); }
    finally { setJLoad(false); }
  };

  const copyLink = () => { navigator.clipboard.writeText(cResult.MeetingLink); setCopied(true); setTimeout(() => setCopied(false), 2000); };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-card border-b border-border">
        <div className="max-w-5xl mx-auto px-6 py-10 text-center">
          ...
          <p className="text-muted-foreground text-sm max-w-md mx-auto">Teachers create a session, students find and join — all in one place.</p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
          ...
          <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
            <div className="px-6 pt-6 pb-5 border-b border-border flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">For teachers — schedule a new class session</p>
              </div>
            </div>
              ...
              {!cResult ? (
                <>
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Class ID"   id="c-cid" type="number" placeholder="e.g. 10" value={cForm.ClassID}   onChange={setC("ClassID")}   error={cErrors.ClassID}   icon={ICONS.book} />
                    <Field label="Section ID" id="c-sid" type="number" placeholder="e.g. 3"  value={cForm.SectionID} onChange={setC("SectionID")} error={cErrors.SectionID} icon={ICONS.book} />
                  </div>
                  <Field label="Subject" id="c-sub" placeholder="e.g. Mathematics" value={cForm.Subject} onChange={setC("Subject")} error={cErrors.Subject} icon={ICONS.book} />
                  {cErr && (
                    <div className="flex items-start gap-2 px-3.5 py-3 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-sm">
                      <Ic d={ICONS.warn} size={15} cls="flex-shrink-0 mt-0.5" />{cErr}
                    </div>
                  )}
                  <Btn loading={cLoad} onClick={handleCreate} className="w-full py-3 mt-1" size="lg">
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-sm font-medium">
                    <Ic d={ICONS.check} size={16} cls="flex-shrink-0" /> Meeting created successfully!
                  </div>
                  <div className="bg-muted rounded-xl border border-border divide-y divide-border overflow-hidden">
                    {[["Subject", cResult.Subject], ["Class", `Class ${cResult.ClassID}`], ["Section", `Section ${cResult.SectionID}`]].map(([k, v]) => (
                  ...
                  <div className="flex flex-col gap-1.5">
                    <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Meeting Link</span>
                    <div className="flex items-center gap-2">
                      <code className="flex-1 min-w-0 text-xs text-primary bg-primary/5 border border-primary/20 rounded-lg px-2.5 py-2 truncate">{cResult.MeetingLink}</code>
                      <button onClick={copyLink} className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center bg-primary/10 hover:bg-primary/20 text-primary transition-colors" title="Copy">
                        <Ic d={copied ? ICONS.check : ICONS.copy} size={14} />
                      </button>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2.5 mt-1">
                    <Btn variant="secondary" onClick={() => { setCResult(null); setCForm({ ClassID: "", SectionID: "", Subject: "" }); }}>New Meeting</Btn>
                    <Btn onClick={() => onEnterRoom(cResult, getUserFromToken() || { name: "Teacher", role: "teacher" })}>
          ...
          <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
            <div className="px-6 pt-6 pb-5 border-b border-border flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">For students — find and enter your class session</p>
              </div>
            </div>

            <div className="p-6 flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-3">
                <Field label="Class ID"   id="j-cid" type="number" placeholder="e.g. 10" value={jForm.ClassID}   onChange={setJ("ClassID")}   icon={ICONS.book} />
                <Field label="Section ID" id="j-sid" type="number" placeholder="e.g. 3"  value={jForm.SectionID} onChange={setJ("SectionID")} icon={ICONS.book} />
              </div>
              <Btn loading={jLoad} onClick={handleFetch} disabled={!jForm.ClassID || !jForm.SectionID} className="w-full py-3" size="lg">
                <Ic d={ICONS.video} size={16} /> Find Meetings
              </Btn>
              {jErr && (
                <div className="flex items-start gap-2 px-3.5 py-3 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-sm">
                  <Ic d={ICONS.warn} size={15} cls="flex-shrink-0 mt-0.5" />{jErr}
                </div>
              )}
              ...
              {meetings.length > 0 && (
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Available Sessions</span>
                    <Badge>{meetings.length} found</Badge>
                  </div>
                  <div className="flex flex-col gap-2 max-h-64 overflow-y-auto pr-0.5">
                    {meetings.map(m => (
                      <div key={m.MeetingID} className="flex items-center gap-3 bg-muted rounded-xl px-4 py-3 border border-border hover:border-primary/30 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1 text-muted-foreground mt-0.5">
                            ...
                            <span className="text-[11px]">{new Date(m.MeetingDate).toLocaleString([], { dateStyle: "medium", timeStyle: "short" })}</span>
                          </div>
                        </div>
                        <Btn size="sm" onClick={() => onEnterRoom(m, getUserFromToken() || { name: "Student", role: "student" })} className="flex-shrink-0">
                          Join
                        </Btn>
                      </div>
                    ))}
        ...
        <p className="text-center text-xs text-muted-foreground mt-8">
          Meetings are powered by <a href="https://meet.jit.si" target="_blank" rel="noreferrer" className="text-primary underline underline-offset-2">Jitsi Meet</a> — no downloads required.
        </p>
      </div>
    </div>
  );
...
function ChatPanel({ apiRef, currentUser, messages, onNewMsg, onClose }) {
  ...
  return (
    <div className="flex flex-col h-full bg-card" style={{ animation: "slideIn .2s ease" }}>
      <div className="flex items-center justify-between px-4 py-3 border-b border-border flex-shrink-0">
        ...
        <button onClick={onClose} className="w-7 h-7 rounded-lg flex items-center justify-center bg-muted hover:bg-accent text-muted-foreground transition-colors">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 flex flex-col gap-2.5">
        ...
        {messages.map(m => (
          <div key={m.id} className={`flex flex-col ${m.self ? "items-end" : "items-start"}`}>
            {!m.self && <span className="text-[11px] font-semibold text-primary mb-0.5 px-1">{m.sender}</span>}
            {m.isSystem ? (
              <div className="text-[11px] text-muted-foreground italic px-3 py-1.5 bg-muted/50 rounded-xl border border-border">{m.text}</div>
            ) : (
              <div className={`max-w-[86%] px-3 py-2 text-sm leading-snug break-words
      ...
      <div className="px-3 py-3 border-t border-border flex-shrink-0">
        ...
        <p className="text-[10px] text-muted-foreground mt-1.5 px-1">Messages are visible to all participants</p>
      </div>
    </div>
  );
}
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2">
        {participants.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-2 text-muted-foreground">
            ...
            <p className="text-sm text-center">Waiting for participants…<br /><span className="text-xs opacity-60">List updates as people join.</span></p>
          </div>
        ) : (
          participants.map(p => (
            <div key={p.id} className="flex items-center gap-3 px-2 py-2.5 rounded-lg hover:bg-muted transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-foreground truncate">{p.displayName ?? "Unknown"}</p>
                <div className="flex items-center gap-1.5 mt-0.5">
                  {p.isLocal && <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-primary/10 text-primary">You</span>}
                  {p.isModerator && <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-600">Host</span>}
                </div>
              </div>
              <div className="flex gap-1">
                <span className={`w-6 h-6 rounded-md flex items-center justify-center ${p.isAudioMuted ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground"}`}>
                  <Ic d={p.isAudioMuted ? ICONS.micOff : ICONS.mic} size={12} />
                </span>
                <span className={`w-6 h-6 rounded-md flex items-center justify-center ${p.isVideoMuted ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground"}`}>
...
// ─── Control Button ───────────────────────────────────────────────────────────
function CtrlBtn({ icon, label, active = true, danger = false, onClick, badge = 0, disabled = false }) {
  return (
    <button onClick={onClick} disabled={disabled} title={label}
      ...
      <span className={`relative w-11 h-11 rounded-xl flex items-center justify-center transition-all duration-200
        ${danger
    ...
    <>
      ...
      <div className="flex flex-col bg-background text-foreground" style={{ height: "100vh" }}>
        ...
        <header className="flex items-center justify-between px-5 py-2.5 bg-card border-b border-border flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Class {meetingData.ClassID} · Section {meetingData.SectionID}</p>
            </div>
            <span className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-destructive/10 border border-destructive/25 text-destructive text-xs font-medium">
              <span className="w-1.5 h-1.5 rounded-full bg-destructive" style={{ animation: "blink 2s infinite" }} />
              LIVE · {fmt(secs)}
            </span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <span className="hidden md:block text-[10px] text-muted-foreground font-mono truncate max-w-[200px]">{meetingData.MeetingName}</span>
          </div>
        </header>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {panel === "people" && (
            <aside className="w-72 flex flex-col border-l border-border flex-shrink-0 overflow-hidden">
              <ParticipantsPanel participants={participants} onClose={() => setPanel(null)} />
            </aside>
          )}
        </div>
        ...
        <footer className="flex items-center justify-between px-5 py-3 bg-card border-t border-border flex-shrink-0">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button onClick={() => setConfirm(true)}
              className="flex flex-col items-center gap-1 group focus:outline-none" title="Leave">
              ...
              <span className="text-[10px] text-muted-foreground group-hover:text-foreground">Leave</span>
            </button>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.5)", backdropFilter: "blur(6px)" }}>
            <div className="bg-card border border-border rounded-2xl p-7 max-w-sm w-full mx-4 text-center shadow-2xl">
              ...
              <p className="text-sm text-muted-foreground mb-6 leading-relaxed">
                You'll be disconnected from <strong className="text-foreground">{meetingData.Subject}</strong>.
              </p>
              <div className="flex gap-2.5">
                <Btn variant="secondary" onClick={() => setConfirm(false)} className="flex-1">Stay</Btn>
                <Btn variant="destructive" onClick={hangUp} className="flex-1">Leave</Btn>
              </div>
            </div>
          </div>
        )}
...
export default function MeetingPage() {
  ...
  return (
    <LandingPage
      onEnterRoom={(meetingData, currentUser) =>
        setRoom({ meetingData, currentUser })
      }
    />
  );
}