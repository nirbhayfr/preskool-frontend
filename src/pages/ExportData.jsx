...
function matchesOperator(cellVal, operator, filterVal) {
  ...
  switch (operator) {
    ...
    case 'Equals':
      return cell === fv
    case 'Starts with':
      return cell.startsWith(fv)
    case 'Ends with':
      return cell.endsWith(fv)
    case 'Is empty':
      return cell === ''
    case 'Is not empty':
      return cell !== ''
  ...
  return (
    ...
    >
      ...
      <Select
        ...
        onValueChange={(v) => {
          setOperator(v)
          if (v === 'Is empty' || v === 'Is not empty') setValue('')
        }}
      >
      ...
      <div className="flex gap-2 pt-0.5">
        ...
        <Button
          size="sm"
          className={`flex-1 h-7 text-xs ${theme.exportBtn}`}
          onClick={() => {
            onApply({ operator, value })
            onClose()
          }}
...
function ActiveFilterChips({ colFilters, orderedCols, onClear, onClearAll, theme }) {
  ...
  return (
    <div
      className={`flex items-center gap-1.5 flex-wrap px-4 py-2 border-b ${theme.iconBg}`}
    >
      ...
      {active.map(([key, f]) => {
        ...
        return (
          ...
          >
            ...
            <button
              onClick={() => onClear(key)}
              className="ml-0.5 opacity-70 hover:opacity-100 hover:text-red-400 transition-colors"
            >
      ...
      <button
        onClick={onClearAll}
        className="text-xs text-muted-foreground hover:text-red-400 transition-colors flex items-center gap-1 ml-1"
      >
  ...
  return (
    <div
      ...
      className={`group relative flex items-center justify-between px-3 py-2.5 rounded-lg border cursor-pointer transition-all duration-150 ${
        isActive
          ? `${theme.configActive} shadow-sm`
          : 'border-border hover:border-muted-foreground/30 hover:bg-accent/60'
      }`}
    >
      {isActive && (
        <div
          className={`absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 ${theme.configBar} rounded-r-full`}
        />
      )}
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <div
              className={`h-6 w-6 rounded-md ${theme.iconBg} flex items-center justify-center`}
            >
              <LayoutTemplate className={`h-3.5 w-3.5 ${theme.iconColor}`} />
            </div>
            {initialData ? 'Edit Config' : 'New Config'}
            <span
              className={`text-xs font-normal px-2 py-0.5 rounded-full border ml-1 ${theme.filterChipBg}`}
            >
              {listName}
            </span>
          </DialogTitle>
        ...
        <div className="space-y-4 py-1">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span
                className={`text-xs px-2 py-0.5 rounded-full border ${theme.filterChipBg}`}
              >
              ...
              <div className="space-y-0.5 px-1">
                {filtered.map((col) => (
                  ...
                  >
                    ...
                    />
                    <Label
                      className={`text-sm font-normal cursor-pointer leading-none ${selected.has(col.key) ? theme.iconColor : ''}`}
                    >
                      {col.label}
                    </Label>
                  </div>
        ...
        <DialogFooter className="gap-2">
          ...
          <Button
            onClick={handleSave}
            disabled={isSaving}
            className={`h-8 text-sm ${theme.exportBtn}`}
          >
  ...
  return (
    <div className="flex items-center justify-between px-4 py-2.5 border-t bg-muted/10 shrink-0">
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span>·</span>
        <span>
          {showing.from}–{showing.to} of{' '}
          <span className={`font-semibold ${theme.iconColor}`}>{total}</span>
        </span>
      </div>
...
function ExportTabPanel({ listName, data, isDataLoading }) {
  ...
  useEffect(() => {
    setPage(1)
  }, [globalSearch, colFilters, selectedCols])

  const filteredRows = useMemo(() => {
    return rows.filter((row) => {
      if (globalSearch.trim()) {
        const q = globalSearch.toLowerCase()
        const hit = orderedCols.some((col) =>
          formatCell(col.key, row[col.key]).toLowerCase().includes(q)
        )
        if (!hit) return false
      }
  ...
  const handleExport = () => {
    ...
    const csvRows = filteredRows.map((r) =>
      orderedCols.map((c) => escapeCsv(formatCell(c.key, r[c.key]))).join(',')
    )
        ...
        <Card className={`shrink-0 border-l-2 ${theme.sidebarAccent}`}>
          <CardHeader className="py-3 px-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                <div
                  className={`h-5 w-5 rounded ${theme.iconBg} flex items-center justify-center`}
                >
                ...
                {configs.length > 0 && (
                  <span
                    className={`text-xs px-1.5 py-0.5 rounded-full border font-medium ${theme.filterChipBg}`}
                  >
                    {configs.length}
                  </span>
                )}
              ...
              <div className="flex items-center gap-1">
                ...
                <Button
                  size="sm"
                  className={`h-6 px-2 text-xs gap-1 ${theme.exportBtn}`}
                  onClick={() => {
                    setEditingConfig(null)
                    setModalOpen(true)
                  }}
          ...
          <CardContent className="px-3 pb-3 pt-0">
            {configsLoading ? (
              ...
              <div className="text-center py-6 space-y-1">
                <LayoutTemplate
                  className={`h-6 w-6 mx-auto opacity-30 ${theme.iconColor}`}
                />
                    ...
                    return (
                      <span
                        ...
                        className={`text-xs px-2 py-0.5 rounded-full border cursor-pointer select-none transition-all ${
                          allOn
                            ? `${theme.filterChipBg} font-medium`
                ...
                {filteredColOptions.length === 0 ? (
                  ...
                  filteredColOptions.map((col) => (
                    ...
                    >
                      ...
                      />
                      <Label
                        className={`text-xs font-normal cursor-pointer leading-none ${selectedCols.has(col.key) ? theme.iconColor : ''}`}
                      >
                        {col.label}
                      </Label>
                    </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {activeFilterCount > 0 && (
                ...
                >
                  ...
                  Clear filters
                  <span className="bg-red-500/20 text-red-400 text-xs px-1.5 py-0.5 rounded-full ml-0.5">
                    {activeFilterCount}
                  </span>
                </Button>
              )}
              ...
              <div className="flex items-center gap-1.5 ml-auto">
                {/* Stats pill */}
                <div
                  className={`flex items-center gap-1.5 text-xs border rounded-lg px-3 py-1.5 ${theme.iconBg} ${theme.filterChipBg}`}
                >
                ...
                <Button
                  ...
                  size="sm"
                  className={`h-8 gap-1.5 text-xs ${theme.exportBtn}`}
                >
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            {isDataLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-muted-foreground gap-3">
                <div
                  className={`h-14 w-14 rounded-full ${theme.iconBg} flex items-center justify-center`}
                >
                  <SlidersHorizontal
                    className={`h-6 w-6 ${theme.iconColor} opacity-60`}
                  />
                </div>
                <div className="text-center">
                  <p className="text-sm font-medium">No columns selected</p>
                  <p className="text-xs text-muted-foreground/70 mt-0.5">
                    Pick columns from the sidebar to see your data
                  </p>
                </div>
              </div>
                ...
                <div className="text-center">
                  ...
                  {activeFilterCount > 0 && (
                    <Button
                      ...
                      size="sm"
                      onClick={clearAllFilters}
                      className="text-xs h-auto p-0 mt-1 text-red-400"
                    >
                      Clear all filters
                    </Button>
                  )}
                ...
                <div className="overflow-auto flex-1">
                  <Table>
                    ...
                    <TableBody>
                      {pagedRows.map((row, i) => (
                        <TableRow
                          key={row.StudentID ?? row.TeacherID ?? row.StaffID ?? i}
    ...
    <div className="flex flex-col h-full p-6 gap-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Configure columns, filter records, and export CSV reports
          </p>
        </div>
      </div>
      ...
      <Tabs defaultValue="Students" className="flex-1 flex flex-col min-h-0">
        <TabsList className="w-fit h-9 p-1">
          ...
          <TabsTrigger
            value="Students"
            className="gap-1.5 text-sm px-4 data-[state=active]:text-blue-500"
          >
          ...
          <TabsTrigger
            value="Teachers"
            className="gap-1.5 text-sm px-4 data-[state=active]:text-emerald-500"
          >
          ...
          <TabsTrigger
            value="Staff"
            className="gap-1.5 text-sm px-4 data-[state=active]:text-orange-500"
          >
